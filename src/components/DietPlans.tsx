...
import diet_plan_cols from "../utility/DietPlanCols";
...
const DietPlans = observer(() => {
  const { dietStore } = useStore();
  const diet_plans = toJS(dietStore.plans);
  const navigate = useNavigate();
  const classes = useStyles();
  ...
  return (
    <div>
      <Header />
      <div className={classes.container}>
      <Stack style={{ margin: '0 0 0 0' }} direction="row" spacing={1} onClick={() => navigate('/add-diet-plan')}>
          <Chip icon={<Add />} label="Create a new Plan" />
        </Stack>
      </div>
      <PlanView plans={diet_plans} />
    </div>
  )
})
